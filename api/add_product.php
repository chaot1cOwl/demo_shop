...
if (isset($_POST["name"]) && isset($_POST["category"]) && isset($_POST["description"]) && isset($_POST["image"]) && isset($_POST["price"])) {
  ...
  $sql = "insert into products (name, description, image, price, id_categories) values (?, ?, ?, ?, ?)";
  // подготавливаем запрос
  $stmt = $db->prepare($sql);
  // добавляем данные к запросу
  $stmt->bind_param('sssii', $_POST['name'], $_POST['description'], $_POST['image'], $_POST['price'], $_POST['category']);
  // выполняем запрос
  $stmt->execute();
} else {
  $ret_data["error"] = 1;
